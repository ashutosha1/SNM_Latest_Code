...
* Some customization done by Dhananjay on 4 May 15
*/
		...
		<span class="userdata">
			<div class="col-xs-12 cb-login">
				...
				<?php if ( in_array( $showUsernameLabel, array( 1, 2, 3, 5 ) ) ) { ?>
					...
					<?php } ?>
				<?php } ?>
				<input id="modlgn-username" type="text" name="username" class="inputbox username-input"  size="<?php echo $usernameInputLength; ?>"<?php echo ( in_array( $showUsernameLabel, array( 4, 5 ) ) ? ' placeholder="' . htmlspecialchars( $userNameText ) . '"' : null ); ?> />
			</span>
			...
			<span id="form-login-password">
				<?php if ( in_array( $showPasswordLabel, array( 1, 2, 3, 5 ) ) ) { ?>
					...
					<?php } ?>
				<?php } ?>
				<input id="modlgn-passwd" type="password" name="passwd" class="inputbox password-input" size="<?php echo $passwordInputLength; ?>"<?php echo ( in_array( $showPasswordLabel, array( 4, 5 ) ) ? ' placeholder="' . htmlspecialchars( CBTxt::T( 'Password' ) ) . '"' : null ); ?>  />
			</span>
                        
                        <button type="submit" name="Submit" class="button cb-login-btn"<?php echo $buttonStyle; ?>>
				...
				<?php if ( in_array( $showButton, array( 0, 1, 4 ) ) ) { ?>
					<?php echo htmlspecialchars( CBTxt::T( 'Go' ) ); ?>
				<?php } ?>
			</button>
	...
	<?php if ( $showForgotLogin || $showRegister ) { ?>
			...
			<span class="hidden-xs triangle-topright"></span>
				...
				<span id="form-login-remember" class="disp-bl">
					<span class="anchor-color" style="padding-left:5px;"><?php echo htmlspecialchars( CBTxt::T( 'Remember Me' ) ); ?></span>
					<span class="lfloat"><input id="modlgn-remember" type="checkbox" name="remember" class="inputbox" value="yes"<?php echo ( $showRememberMe == 3 ? ' checked="checked"' : null ); ?> /></span>
			...
			<?php echo modCBLoginHelper::getPlugins( $params, $type, 'afterButton', 'span', 1, null, '&nbsp;' ); ?>
				...
				<span id="form-login-forgot">
					<a  href="<?php echo $_CB_framework->viewUrl( 'lostpassword', true, null, 'html', $secureForm ); ?>">
						...
						<span class="anchor-color">	<?php echo CBTxt::T( 'Forgot Login?' ); ?></span>
				...
				<span id="form-login-register">
					<a href="<?php echo $_CB_framework->viewUrl( 'registers', true, null, 'html', $secureForm ); ?>">
						...
						<?php if ( in_array( $params->get( 'show_newaccount', 1 ), array( 1, 3 ) ) ) { ?>
							<span class="sign-up-btn btn"><?php echo CBTxt::T( 'SIGN_UP UE_REGISTER', 'Sign up' ); ?></span>
						<?php } ?>
					</a>
				</span>
				&nbsp;
