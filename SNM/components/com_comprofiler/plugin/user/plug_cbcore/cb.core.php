	...
	public function getEmbed( $field, $user, $value, $thumb = false ) {
		...
		if ( $urlDomain ) {
			...
			if ( $mimeType && in_array( $mimeType, $this->allowedMimeTypes() ) ) {
				$embed				=	'<video width="' . (int) $width . '" height="' . (int) $height . '" src="' . htmlspecialchars( $value ) . '" type="' . htmlspecialchars( $mimeType ) . '" preload="auto" class="cbVideoField"></video>';
			}
	...
	public function getEmbed( $field, $user, $value, $thumb = false ) {
		...
		if ( $urlDomain ) {
			...
			if ( $embed ) {
				...
				if ( ! $JS_loaded++ ) {
					$_CB_framework->outputCbJQuery( "$( '.cbAudioField' ).mediaelementplayer({ isVideo: false });", 'media' );
				}
			}
		}
