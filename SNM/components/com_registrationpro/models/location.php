...
class registrationproModelLocation extends JModelLegacy
{
	...
	function _buildContentWhere()
	{
		...
		$event_location 	= $this->_db->escape(trim(JRequest::getVar('txtEventLocation','','POST')));
		...
		$event_title 		= $this->_db->escape( trim(JString::strtolower( $event_title ) ) );
		...
		$event_location 	= $this->_db->escape( trim(JString::strtolower( $event_location ) ) );
		...
		if ($event_title) {
			$where[] = ' LOWER(a.titel) LIKE '.$this->_db->quote( $this->_db->escape('%'.$event_title.'%',false));
			//$where[] = ' LOWER(a.titel) LIKE \'%'.$event_title.'%\' ';
		}
		
		if ($event_start_date) {
			$where[] = ' LOWER(a.dates) LIKE '.$this->_db->quote( $this->_db->escape('%'.$event_start_date.'%',false));
			//$where[] = ' LOWER(a.dates) LIKE \'%'.$event_start_date.'%\' ';
		}
		
		if ($event_end_date) {
			$where[] = ' LOWER(a.enddates) LIKE '.$this->_db->quote( $this->_db->escape('%'.$event_end_date.'%',false));
			//$where[] = ' LOWER(a.enddates) LIKE \'%'.$event_end_date.'%\' ';
		}
		
		if ($event_location) {
			$where[] = ' LOWER(l.club) LIKE '.$this->_db->quote( $this->_db->escape('%'.$event_location.'%',false));
