...
if ($this->regproConfig['collapse_categories'])
	$collapsestatus = $this->regproConfig['collapse_categories'];
else $collapsestatus = 0;
...
<?php $regpro_header_footer = new regpro_header_footer; $regpro_header_footer->regpro_header($this->regproConfig); ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="eventlisting">
	...
	<tr>
		<th width="18%">
			<?php echo JHTML::_('grid.sort', '<b>'. JText::_('EVENTS_FRONT_TITLE') .'<b/>', 'a.titel', $this->lists['order_Dir'], $this->lists['order'] ); ?>
		</th>
	...
	?>
		...
		?>
			<tr <?php echo $background;?>>
				<td colspan="<?php echo $colspan; ?>" <?php echo $background;?>>
		...
		?>
			<td valign="top"><a href="<?php echo $detaillink ; ?>"><?php echo $row->titel; ?></a></td>
				...
				<?php
					...
					if (( $this->regproConfig['showurl'] == 1) && (!empty($row->url))) {
						if(strtolower(substr($row->url, 0, 7)) == "http://") {
							...
							$location .= '<a href="http://'.$row->url.'" target="_blank">'.$row->club.'</a>';
						}
					} else {
						$location .= $row->club;
					}
					...
					//echo $row->club.", ".$row->city." (".$row->country.")";
					echo $location;
				?>
			</td>
		<?php } ?>
	...
	?>
			<tr><td colspan="<?php echo $colspan; ?>" style="text-align:center"><?php echo JText::_('NO_EVENT_RECORD'); ?></td></tr>
	<?php
		}
	?>
	</tbody>
...
<!--<br />
<div style="text-align:center;"><?php echo $this->pageNav->getPagesLinks(); ?> <br/> <?php echo $this->pageNav->getLimitBox(); ?></div>-->
