...
$db  = JFactory::getDbo();
$sql = "SELECT `manifest_cache` FROM `#__extensions` WHERE `type`='component' AND `element`='com_obrss'";
$db->setQuery( $sql );
$res      = $db->loadResult();
$manifest = new JRegistry( $res );
$version  = $manifest->get( 'version' );
define( "GENERATOR", "obRSS " . $version );
...
if ( class_exists( 'EnclosureItem' ) != true ) {
 ...
 * @since  1.3
 */
if ( class_exists( 'FeedImage' ) != true ) {
 ...
 * @author Kai Blankenhorn <kaib@bitfolge.de>
 */
if ( class_exists( 'UniversalFeedCreator' ) != true ) {
...
class FeedDate {
	 ...
	 * }
	 */
	function FeedDate( $dateString = "", $offsetrss = 0 ) {
		$this->offsetrss = $offsetrss;
		if ( $dateString == '' ) {
...
// 			}
	...
	function rfc822() {
		...
		if ( $this->offsetrss ) {
			$date = str_replace( $sOffset, $jOffset, $date );
		}

		return $date;
		//return gmdate("r",$this->unix);
		if ( $this->offsetrss != "" ) {
			date_default_timezone_set( $this->offsetrss );
		}
		$date = date( "r", $this->unix );
 ...
 * @author Kai Blankenhorn <kaib@bitfolge.de>
 */
if ( class_exists( 'RSSCreator10' ) != true ) {
	class RSSCreator10 extends FeedCreator {
		 ...
		 */
		function createFeed() {
			$pr_offsetrss = $this->params->rss;
			$date_format  = $pr_offsetrss->date_format;
			...
			$now = new FeedDate( '', $pr_offsetrss->offsetrss );
			$feed .= "		<dc:date>" . htmlspecialchars( $now->$date_format() ) . "</dc:date>\n";
			$feed .= "		<items>\n";
			$feed .= "			<rdf:Seq>\n";
			...
			for ( $i = 0; $i < count( $this->items ); $i ++ ) {
				...
				if ( $this->items[ $i ]->date != null ) {
					$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
					$feed .= "		<dc:date>" . htmlspecialchars( $itemDate->$date_format() ) . "</dc:date>\n";
				}
 ...
 * */
if ( class_exists( 'OBJSONCreator' ) != true ) {
		...
		';
			...
			';
				...
				if ( $this->items[ $i ]->date != "" ) {
					$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
					$feed .= '
								"pubDate":"' . htmlspecialchars( $itemDate->$date_format() ) . '",
				';
				}
				...
				$item_description = str_replace( "]]>", "", str_replace( "<![CDATA[", "", $item_description ) );
...
// 			';
				$additionalElements = $this->items[ $i ]->additionalElements;
				if ( $additionalElements != null ) {
					foreach ( $additionalElements as $a_key => $a_value ) {
						$feed .= '
								"' . $a_key . '":"' . $a_value . '",
					';
					}
				}
				$json_enclosure = $this->items[ $i ]->enclosure;
				//echo'<pre>';print_r($this->validateJSONFeed( json_encode($json_enclosure) ));die;
				if( $json_enclosure ){
				$feed .= '
								"enclosure":[' . json_encode($json_enclosure) . '],
			';
				}
				$feed .= '
								"description":' . $this->validateJSONFeed( $item_description ) . '
			';
				$categories = $this->items[ $i ]->category; //echo'<pre>';print_r($this->items[$i]->additionalElements);die;
		...
		';
			...
			';
				...
				if ( $this->items[ $i ]->date != "" ) {
					$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
					$feed .= '
								"publishedDate":"' . htmlspecialchars( $itemDate->$date_format() ) . '",
				';
				}
				// contentSnippet - A snippet (< 120 characters) version of the content attribute. The snippet does not contain any HTML tags.
				$item_description = $this->items[ $i ]->getDescription();
 ...
 * @author Kai Blankenhorn <kaib@bitfolge.de>
 */
if ( class_exists( 'RSSCreator091' ) != true ) {
	class RSSCreator091 extends FeedCreator {
		...
		function createFeed() {
			...
			# namespaces
			$feed .= " xmlns:atom=\"http://www.w3.org/2005/Atom\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:content=\"http://purl.org/rss/1.0/modules/content/\" xmlns:media=\"http://search.yahoo.com/mrss/\"";
			$feed .= ">\n";
			$feed .= "	<channel>\n";
			...
			$feed .= "		<link>" . $this->link . "</link>\n";
			$now = new FeedDate( '', $pr_offsetrss->offsetrss );
			$feed .= "		<lastBuildDate>" . htmlspecialchars( $now->$date_format() ) . "</lastBuildDate>\n";
			...
			if ( $this->pubDate != "" ) {
				$pubDate = new FeedDate( $this->pubDate, $pr_offsetrss->offsetrss );
				$feed .= "		<pubDate>" . htmlspecialchars( $pubDate->$date_format() ) . "</pubDate>\n";
			}
			if ( $this->category != "" ) {
			...
			for ( $i = 0; $i < count( $this->items ); $i ++ ) {
				...
				*/
				...
				if ( $this->items[ $i ]->date != "" ) {
					$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
					$feed .= "			<pubDate>" . htmlspecialchars( $itemDate->$date_format() ) . "</pubDate>\n";
				...
				if ( $this->RSSVersion == "2.0" && $this->items[ $i ]->enclosure != null ) {
					...
					$feed .= "\"/>\n";
				} elseif ( $this->RSSVersion == "2.0" && $this->items[ $i ]->enclosures != null ) {
					$feed .= "<media:group>\n";
					foreach ( $this->items[ $i ]->enclosures as $ec ) {
						$feed .= "<media:content url=\"";
						$feed .= $ec->url;
						$feed .= "\" length=\"";
						$feed .= $ec->length;
						$feed .= "\" type=\"";
...
class PIECreator01 extends FeedCreator {
	...
	function createFeed() {
		...
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			...
			$feed .= "		<link>" . htmlspecialchars( $this->items[ $i ]->link ) . "</link>\n";
			$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
			$feed .= "		<created>" . htmlspecialchars( $itemDate->$date_format() ) . "</created>\n";
			$feed .= "		<issued>" . htmlspecialchars( $itemDate->$date_format() ) . "</issued>\n";
			$feed .= "		<modified>" . htmlspecialchars( $itemDate->$date_format() ) . "</modified>\n";
...
class AtomCreator10 extends FeedCreator {
	...
	function createFeed() {
		...
		$now = new FeedDate( '', $pr_offsetrss->offsetrss );
		$feed .= "	<updated>" . htmlspecialchars( $now->$date_format() ) . "</updated>\n";
		if ( $this->editor != "" ) {
			$feed .= "	<author>\n";
		...
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			...
			$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
			$feed .= "		<published>" . htmlspecialchars( $itemDate->$date_format() ) . "</published>\n";
			$feed .= "		<updated>" . htmlspecialchars( $itemDate->$date_format() ) . "</updated>\n";
...
class AtomCreator03 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed .= "	<id>" . htmlspecialchars( $this->link ) . "</id>\n";
		$now = new FeedDate( '', $pr_offsetrss->offsetrss );
		$feed .= "	<modified>" . htmlspecialchars( $now->$date_format() ) . "</modified>\n";
		if ( $this->editor != "" ) {
			$feed .= "	<author>\n";
			$feed .= "		<name>" . $this->editor . "</name>\n";
			if ( $this->editorEmail != "" ) {
...
class MBOXCreator extends FeedCreator {
	...
	function createFeed() {
		...
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			...
			$feed .= "Date: " . $itemDate->$date_format() . "\n";
...
class OPMLCreator extends FeedCreator {
	...
	function createFeed() {
		...
		$feed .= "		<title>" . htmlspecialchars( $this->title ) . "</title>\n";
		if ( $this->pubDate != "" ) {
			$date = new FeedDate( $this->pubDate, $pr_offsetrss->offsetrss );
			$feed .= "		 <dateCreated>" . $date->$date_format() . "</dateCreated>\n";
		}
		if ( $this->lastBuildDate != "" ) {
			$date = new FeedDate( $this->lastBuildDate, $pr_offsetrss->offsetrss );
			$feed .= "		 <dateModified>" . $date->$date_format() . "</dateModified>\n";
		}
		if ( $this->editor != "" ) {
...
class GoogleSiteMapIndex extends FeedCreator {
	...
	function createFeed() {
		...
		for ( $i = 0; $i < $total; $i ++ ) {
			...
			$feed .= "		<priority>" . $this->params->sitemap->priority . "</priority>\n";
			$feed .= "		<changefreq>" . $this->params->sitemap->frequency . "</changefreq>\n";
			if ( $this->items[ $i ]->date != "" ) {
				$itemDate = new FeedDate( $this->items[ $i ]->date, $pr_offsetrss->offsetrss );
				$feed .= "		<lastmod>" . htmlspecialchars( $itemDate->$date_format() ) . "</lastmod>\n";
			}
			$feed .= "	</url>\n";
		}
		$feed .= "</urlset>\n";
