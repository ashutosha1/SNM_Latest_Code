...
class registrationproModelField extends JModelLegacy
{
	...
	function _initData()
	{
		if (empty($this->_data))
		{
			...
			$field->conditional_field	= null;
			$field->conditional_field_values = null;
			$field->conditional_field_name = null;				
			$field->fees_field		= null;
			$field->fees 			= null;
			$field->fees_type 		= null;
	...
	function store($data)
	{
		...
		$row->checkin();
		$row->reorder('form_id = '.(int) $row->form_id);
	...
	function copyfields($copied_formid, $assign_formid)
	{
		...
		foreach($fields as $key=>$value){
			...
			$row->display_type	=	$fields[$key]->display_type;
			$row->conditional_field	=	$fields[$key]->conditional_field;
			$row->conditional_field_values	=	$fields[$key]->conditional_field_values;
			$row->conditional_field_name	=	$fields[$key]->conditional_field_name;
			$row->fees_field	=	$fields[$key]->fees_field;
			$row->fees			=	$fields[$key]->fees;
			$row->fees_type		=	$fields[$key]->fees_type;
	...
	function getconditionalfields($form_id)
	{
		// get selectboxes, checkboxes and radio buttons only
		$query = "SELECT id AS value, title AS text FROM #__registrationpro_fields WHERE (inputtype = 'radio' || inputtype = 'checkbox' || inputtype = 'multicheckbox' || inputtype = 'select' || inputtype = 'country' || inputtype = 'state') AND published = 1 AND fees_field = 0 AND form_id =".$form_id;
		$this->_db->setQuery($query);
		$fields  = $this->_db->loadObjectList();
		return $fields;
	}
	...
	{
		// get all fields groups
		$query = "SELECT name AS value, title AS text FROM #__registrationpro_fields WHERE published = 1 AND (inputtype = 'text' OR  inputtype = 'password' ) AND form_id =".$form_id;
		$this->_db->setQuery($query);
		$all_fields  = $this->_db->loadObjectList();
		return $all_fields;
	}	
	
	function getFieldValues($form_id, $field)
	{						
		$query = "SELECT * FROM #__registrationpro_fields WHERE form_id=".$form_id." AND id=".$field;
		$this->_db->setQuery($query);
	...
	{
		$query = "SELECT name FROM #__registrationpro_fields WHERE id=".$fieldid;
		$this->_db->setQuery($query);
	...
	// **************************************** Joomla profile Fields functions ***************************************************
	
	function publishprofilefield($cid, $form_id,$publishfield="")
	{					
		//delete from registrationpro_cbfields where cbfields = cid and form_id = get[form_id]
		$query = "DELETE FROM #__registrationpro_cbfields WHERE form_id = $form_id AND corefield_id = '$cid'";
		$this->_db->setQuery($query);
		$this->_db->query();
		
		if($publishfield){
			//insert
			$query = "INSERT INTO #__registrationpro_cbfields (form_id, corefield_id) VALUES ($form_id,'$cid')";
			$this->_db->setQuery($query);
			$this->_db->query();			
		}		
	}
