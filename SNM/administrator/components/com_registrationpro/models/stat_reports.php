...
class registrationproModelStat_Reports extends JModelLegacy
{
	var $_data = null;
	var $_total = null;
	...
	function getEventReportData()
	{
		...
		$month 			= $mainframe->getUserStateFromRequest( $option.'.report_month', 'month', '', 'int' );
		$year			= $mainframe->getUserStateFromRequest( $option.'.report_year', 'year', '', 'int' );
		$category		= $mainframe->getUserStateFromRequest( $option.'.report_category', 'cat', '' );
		$event_id		= $mainframe->getUserStateFromRequest( $option.'.report_eventid', 'event_id', '');
		$payment_status	= $mainframe->getUserStateFromRequest( $option.'.payment_status', 'payment_status', '', 'int' );
		//echo 'Category : ';print_r($event_id);die;
		...
		/* Code customized by SUSHIL on 13-02-2015 */
		if(count($category) == 1)
		{
			if($category[0] != 0)
			{
				$category_status = " AND (e.catsid =".$category[0].")";
				
				if(count($event_id) == 1) {
					if($event_id[0] != 0)
					{
						$category_status = $category_status . " AND (e.id =".$event_id[0].")";
					}
				}else{
					$category_status = $category_status . " AND (e.id  IN ('".implode(',',$event_id)."'))";
				}
			}else{
				$category_status = "";
				if(count($event_id) == 1) {
			...
			$category_status = " AND (e.catsid IN ('".implode(',',$category)."'))";
	...
	function getUserinfoForExcelReport()
	{
		...
		$month 			= $mainframe->getUserStateFromRequest( $option.'.report_month', 'month', '', 'int' );
		$year			= $mainframe->getUserStateFromRequest( $option.'.report_year', 'year', '', 'int' );
		$category		= $mainframe->getUserStateFromRequest( $option.'.report_category', 'cat', '', 'array' );
		$event_id		= $mainframe->getUserStateFromRequest( $option.'.report_eventid', 'event_id', '', 'array');
		$payment_status	= $mainframe->getUserStateFromRequest( $option.'.payment_status', 'payment_status', '', 'int' );
		...
		if(count($category) > 0 && $category[0] > 0) {
			$imp_category = implode(',' , $category);
			$category_status = " AND (e.catsid IN (".$imp_category."))";
			if(count($event_id) > 0 && $event_id[0] > 0){
				//echo "<pre>";print_r($event_id);
				$imp_event_id = implode(',' , $event_id);
				$category_status = $category_status . " AND (e.id IN (".$imp_event_id."))";
			}
		}else{
			$category_status = "";
		}
