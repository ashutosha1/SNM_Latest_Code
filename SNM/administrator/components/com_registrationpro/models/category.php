	...
	{			
		if($this->_id) {
			$query = "SELECT id AS value, catname AS text"
				...
				. "\nWHERE publishedcat = 1 AND parentid !=".$this->_id." AND id != ".$this->_id
				. "\nORDER BY catname";
		}else{
			$query = "SELECT id AS value, catname AS text"
				. "\nFROM #__registrationpro_categories"
				. "\nWHERE publishedcat = 1 ORDER BY catname";
		}
		
		$this->_db->setQuery($query);
		$allcategories = $this->_db->loadObjectList();
