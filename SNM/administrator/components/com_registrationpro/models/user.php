...
class registrationproModelUser extends JModelLegacy
{
	...
	function getTransaction_details($rid = 0)
	{
		...
		$query = "SELECT t.*, edt.event_discount_amount, edt.event_discount_type FROM #__registrationpro_transactions t"
				...
				. "\n WHERE t.reg_id = $rid";
		/*$query = "SELECT t.*, edt.event_discount_amount, edt.event_discount_type, afff.additional_field_name, afff.additional_field_fees"
				." FROM #__registrationpro_transactions t"
				. "\nLEFT JOIN #__registrationpro_event_discount_transactions AS edt ON t.id = edt.trans_id"
				. "\nLEFT JOIN #__registrationpro_additional_from_field_fees AS afff ON afff.reg_id = t.reg_id"
				. "\n WHERE t.reg_id = $rid";*/		
		$this->_db->setQuery($query);
	...
	function getAdditional_form_fees($userid = 0)
	{
		$query = "SELECT * FROM #__registrationpro_additional_from_field_fees WHERE reg_id=".$userid;
		$this->_db->setQuery($query);
	...
	// get session fees 
	function getSession_fees($userid = 0)
	{
		$query = "SELECT * FROM #__registrationpro_session_transactions WHERE reg_id=".$userid;
		$this->_db->setQuery($query);
		$data = $this->_db->loadObjectList();
		return $data;
	}	
	
	function getEvents($event_id = 0)
	{		
		$query = "SELECT id AS value, titel AS text"
			. "\nFROM #__registrationpro_dates"
			. "\nWHERE published = 1 AND id != ".$event_id
			. "\nORDER BY titel";
