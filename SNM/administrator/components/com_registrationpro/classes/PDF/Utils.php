...
class CRM_Utils_PDF_Utils {
  ...
  static function html2pdf(&$text, $fileName = 'civicrm.pdf', $output = FALSE, $pdfFormat = NULL) {
    ...
    $orientation = CRM_Core_BAO_PdfFormat::get('orientation', $format);
    $metric      = CRM_Core_BAO_PdfFormat::get('metric', $format);
    $t           = CRM_Core_BAO_PdfFormat::get('margin_top', $format);
    $r           = CRM_Core_BAO_PdfFormat::get('margin_right', $format);
    $b           = CRM_Core_BAO_PdfFormat::get('margin_bottom', $format);
    $l           = CRM_Core_BAO_PdfFormat::get('margin_left', $format);
...
<html>
  ...
  static function _html2pdf_wkhtmltopdf($paper_size, $orientation, $html, $output, $fileName) {
    ...
    $config = CRM_Core_Config::singleton();
    $snappy = new KnpSnappyPdf($config->wkhtmltopdfPath);
