<?php
/**
 * @version          $Id: upgrade.php 55 2014-01-22 03:50:36Z thongta $
...
class JLORDRSSModelUpgrade extends obModel {
	...
	function getLog() {
		...
		foreach ( $logs as $log ) {
			...
			$log = substr( $log, 0, - 5 );
			$act = substr( $log, - 7 );
			if ( ! in_array( $act, array( 'upgrade', 'restore' ) ) ) {
				continue;
			}
			if ( $act == 'upgrade' ) {
				$reSto = $log;
	...
	function pk_invalid( $extDir, $curPc, $curVer, $pVer, $ugr = true ) {
		...
		if ( $xml->loadFile( $pathVer ) ) {
			if ( ! $xml->document->url
				...
				if ( ( $ugr && $curVer != $preVer[0]->_data ) || ( ! $ugr && $pVer != $ver[0]->_data ) ) {
					$err = 6;
				}
			}
		} else {
	...
	function doUpdate( $p_file, $pc = '', $curVer = '', $preVer = '', $ugr = true ) {
		...
		foreach ( $ugrFiles as $file ) {
			...
			if ( is_file( $fDest ) ) {
				...
				if ( JFile::copy( $fDest, $bakFile ) ) {
					$urgInfo .= '<br />File: ' . $fname . ' [ <b><span style="color:#ff9933">Backup</span></b> ]';
				}
			} else {
				$update = false;
				$this->CreateFolder( $fDest );
			}
			if ( JFile::copy( $file, $fDest ) ) {
				$urgInfo .= '<br />File: ' . $fname . ' [ <b><span style="color:#339933">' . ( $update ? 'Updated' : 'Created' ) . '</span></b> ]';
			} else {
				$urgInfo .= '<br /><span style="color:#ff0033">Can\'t update file: ' . $fname . '</span>';
			}
		}
		...
		if ( $this->zipFolder( $bakDU ) ) {
			...
			echo '<b><span style="color:#ff0033">Can\'t create Zip file backup</span>';
		}
		if ( $ugr ) {
	...
	function exeSqlUgr( $extDir ) {
		...
		if ( is_file( $incPath ) ) {
			...
			foreach ( $qrys as $qry ) {
				...
				$sqlUgr .= '<br/>';
				if ( ! $db->query() ) {
					$sqlUgr .= '<span style="color:#cc0000;"><b>[Error]: </b>' . $db->_errorMsg . '</span>';
				} else {
					$sqlUgr .= '<b><span style="color:#339933">[Success]:</span> </b>' . $qry;
				}
			}
			$sqlUgr .= '<br /><b>Update Datababe DONE!</b>';
		} else {
			$sqlUgr = 'not exist upgrade.sql file.';
		}
	...
	function exeScriptUgr( $extDir ) {
		...
		if ( is_file( $incPath ) ) {
			...
			$scriptInfo = '<br />Not exist upgrade.php file.';
		}
	...
	function getCheckversion() {
		...
		if ( ! $fp ) {
			...
			for ( $i = 0; $i < count( $arrLastVer ); $i ++ ) {
				if ( ( isset( $arrVer[ $i ] ) ) && ( intval( $arrVer[ $i ] ) > intval( $arrLastVer[ $i ] ) ) ) {
					return false;
				}
				if ( ( ! isset( $arrVer[ $i ] ) ) || ( intval( $arrVer[ $i ] ) < intval( $arrLastVer[ $i ] ) ) ) {
					return true;
